	...
	public ResponseEntity<Integer> checkEmpId(@RequestBody EmpVO emp, HttpServletRequest request) {
		...
		if(empInfo == null) {
			...
			String cont = "<div style=\"width:70%; margin : 0 auto; height : 70%;\">\r\n"
					...
					+ "		<p>아래의 비밀번호 재설정 버튼을 클릭하여<br>비밀번호를 재설정하는 페이지로 이동할 수 있습니다.<br>		"
					+ "<br>본인이 새로운 비밀번호를 요청하지 않았다면 이 메일을 무시해주세요.</p>\r\n"
					+ "        <p>\r\n"
					+ "	</div>\r\n"
					+ "    <a href=\""+ipAddr+"/cmn/login/reset/"+token+"\" style=\"text-decoration : none;\">"
					+ "<button type=\"button\" style=\"cursor : pointer; color : #fff; background-color : #1abc9c; border-color : #1abc9c; "
					+ "display: block; width: 30%; margin : 0 auto; padding : 2%; font-weight : bold; vertical-align : middle; border-radius : 10px; "
					+ "justify-content : flex-start; height : 20%;\">비밀번호 설정</button></a>\r\n"
					+ "</div>";
